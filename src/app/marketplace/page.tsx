...
export default function MarketplacePage() {
  ...
  const filteredListings = listings.filter(ticket => {
    const matchesSearch = ticket.to.toLowerCase().includes(searchQuery.toLowerCase()) || ticket.from.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesPrice = ticket.price <= maxPrice;
        ...
        <aside className="w-full lg:w-80 space-y-[var(--spacing-md)] shrink-0 z-10">
          <GlassCard className="p-[var(--spacing-md)] rounded-xl border border-white/10">
            ...
            <div className="space-y-[var(--spacing-sm)]">
              {/* Search */}
              <div>
                <label className="font-inter text-sm font-medium text-[var(--color-on-surface-variant)] mb-[var(--spacing-xs)] block">Destination / Origin</label>
              ...
              {/* Price Range */}
              <div>
                <label className="font-inter text-sm font-medium text-[var(--color-on-surface-variant)] mb-[var(--spacing-xs)] block">Max Price: ${maxPrice}</label>
                <input 
                  value={maxPrice}
                  onChange={(e) => setMaxPrice(Number(e.target.value))}
                  min="100"
                  max="5000"
                  className="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer accent-[var(--color-tertiary)]" 
                  type="range" 
                />
              ...
              <div>
                ...
                <select className="w-full bg-white/5 border border-white/20 rounded-lg px-[var(--spacing-sm)] py-2 text-[var(--color-on-surface)] outline-none focus:border-[var(--color-secondary)] appearance-none cursor-pointer">
                  <option className="bg-[var(--color-surface)] text-white">Cheapest First</option>
                  <option className="bg-[var(--color-surface)] text-white">Closest Date</option>
                  <option className="bg-[var(--color-surface)] text-white">Trending</option>
                </select>
              </div>
        ...
        <section className="flex-grow space-y-[var(--spacing-md)] z-10 pb-20">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-[var(--spacing-sm)]">
            ...
            <div className="flex gap-[var(--spacing-xs)]">
              <span className="px-[var(--spacing-sm)] py-[var(--spacing-xs)] bg-white/5 border border-white/20 rounded-full font-inter text-xs text-[var(--color-secondary)] flex items-center gap-[var(--spacing-xs)] uppercase tracking-widest font-bold">
                <span className="w-2 h-2 bg-[var(--color-secondary)] rounded-full animate-pulse"></span> {filteredListings.length} Listings
              </span>
            </div>
          </div>
          ...
          {filteredListings.length === 0 ? (
            <GlassCard className="p-[var(--spacing-xl)] text-center">
              ...
              <h3 className="font-outfit text-2xl font-bold text-[var(--color-on-surface)] mb-2">No tickets found</h3>
                ...
                >
                  <div className="flex justify-between items-start mb-[var(--spacing-md)]">
                    ...
                    <div className="bg-[var(--color-secondary-container)]/30 border border-[var(--color-secondary)]/20 px-[var(--spacing-sm)] py-1 rounded-full flex items-center gap-[var(--spacing-xs)]">
                      ...
                      <span className="text-[10px] font-bold text-[var(--color-secondary)] uppercase tracking-widest hidden sm:inline-block">Verified</span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      <div className="font-inter text-xs text-[var(--color-on-surface-variant)] line-through">Orig: ${ticket.origPrice.toLocaleString()}</div>
                      <div className="font-outfit text-2xl font-bold text-[var(--color-tertiary)]">${ticket.price.toLocaleString()}</div>
                    </div>
